...
def read_csv_auto_delimiter(filename, max_rows=None):
    """
    Читает CSV файл с автоматическим определением разделителя
    """
...
def read_file_auto(filename, max_rows=None):
    """
    Универсальное чтение файла с автоопределением формата.
    Возвращает (DataFrame, delimiter, encoding). Для Excel/ODS delimiter и encoding равны None.
    """
...
def analyze_file_structure(filename):
    """
    Анализирует структуру файла без загрузки в память (для отладки)
    """
    ext = os.path.splitext(filename)[1].lower()
